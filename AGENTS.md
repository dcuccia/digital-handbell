...
- **September 24 resumed:** the owner explicitly resumed work. Start from
  ...
  is saved as `e954884c...` with matching manifest. Its filled recovery
  ...
  **+3V3, not MAIN GND**; never repair a ground based on that invalid anchor.
  ...
  lifecycle bridge to immutable CAD inputs. Post-IMU triage reconciles
  38 opens across 24 nets. The local ordinary-via return for the isolated
  C6/C7/C8/C13/C17 GND group is blocked by modeled rear BT1 metal.
  Its initial attempt and two corrections are exhausted; no PCB changed.
  Sol is idle. A longer F escape to an off-contact ordinary via requires
  a new bounded engineering disposition; do not repeat the local screen,
  relax contact clearance or substitute filled vias. No VCORE edge or
  other routing is released. See the closure-plan top.
