...
"""Candidate-local native filled-island extension; never substitutes zone outlines.

Pads/tracks/vias and their physical identities use the unchanged pure checker.
Each zone island has a synthetic item identity, distinct from its parent zone.
Only geometric contact can join islands, including islands of the same zone.
"""
...
def filled_islands(pcb, zone, layer):
    """Use exact native linked-hole boundaries, without integer normalization."""
...
class IslandItem:
    """Small BOARD_CONNECTED_ITEM-compatible adapter for exactly one island.

    TransformShapeToPolygon supports the old pure graph's terminal-cut routine.
    It exposes the filled copper, including holes, never the parent zone outline.
    """
...
class CopperGraph(PrimitiveGraph):
    def __init__(self, pcb, path):
        ...
        # The primitive graph already checked primitive/primitive contacts.
        # Parent zone UUIDs deliberately have no aggregate vertices or barrels.
        done = set()
        for a in zone_keys:
            aa = self.vertices[a]
...
def self_test(pcb):
    """In-memory geometry checks only: no candidate loading, filling or saving."""
