...
"""Measure the final printed-bell-quote-candidate PCB without changing inputs.

Run ONLY after SES import AND this package's native checker have finished:
  python -B measure.py --pcb-sha256 <exact final 64 hex digits>

Writes only this package's reports/power-path-measurements.{json,md}. Requires
KiCad 10.0.6 and numpy; installs nothing. --help does not import native tools.
No DRC, generator, old-package writer, or checker main is invoked. The source
is printed-bell-power-rework, identified by authoritative raw-file hashes, not
a mutable source report folder or pending publication revision.
...
Adapted from tools/analyze_printed_bell_power_paths.py (MIT). Geometry and
numpy initialization are owned here; no imported module globals are patched.
"""
...
SLICE_MM = .25
NECK_WIDTH_MM = .35
np = None                        # This dedicated copy owns its lazy numpy state.
EXPECTED_NETS = {
    ...
    "bulk_to_amp7": "VAMP", "bulk_to_amp8": "VAMP",
    "amp_to_bulk_ground": "GND",
    "input_cap_return": "GND",
}
...
ASSUMPTIONS = [
    ...
    "No IPC-2152 or 10 C temperature-rise claim, ampacity, functional, fault, charging, fabrication, "
    "mechanical, live-cell or child-use signoff follows. Native diagnostics remain release gates.",
    "The actual final native file, not intended SES groups or former via positions, owns the result. "
    "Bias/sense tracks are classified by physical endpoints and chosen-walk ownership.",
    "Two off-pad return vias, when present, are inventoried individually. Neither their presence "
    "nor four outside Q5 fanouts establishes equal current division or final network performance.",
]
...
def load_checker():
    """Import only the new checker and its pure native helpers, never a generator."""
    spec = importlib.util.spec_from_file_location("quote_candidate_measure_checker", HERE / "check.py")
    require(spec is not None and spec.loader is not None, "Cannot load candidate checker")
    check = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(check)
    require(check.HERE == HERE and check.ROOT == ROOT and check.SOURCE == SOURCE,
            "Candidate checker package scope changed")
    return check


def coherent_inputs(check, expected):
    """Explicit authoritative inputs plus this candidate's exact native evidence."""
    require(check.PINNED == PINNED_SOURCE, "Checker source revision differs from this measurement tool")
    for name, digest in PINNED_SOURCE.items():
        require(sha(SOURCE / name) == digest, "Pinned authoritative source changed: " + name)
    inputs = check.input_bindings()
    evidence = {
        REPORTS / "completion-review.json", REPORTS / "completion-review.md",
        REPORTS / "native-input-bindings.json",
        *(REPORTS / name for name in check.NATIVE_REPORTS),
    }
    bindings = dict(inputs)
    bindings.update({str(path.relative_to(ROOT)): sha(path)
                     for path in sorted(evidence | {Path(__file__)})})
    build = read_json(HERE / "completion-build.json")
    manifest = read_json(HERE / "placement-manifest.json")
    require(sha(HERE / "handbell.kicad_pcb") == expected
            == build["protected_files"]["handbell.kicad_pcb"] == manifest["generated_pcb_sha256"],
            "Final PCB/build/manifest hash mismatch; wait for completed import and native checks")
    require(build["generator_sha256"] == sha(HERE / "route.py"), "Generator/build binding changed")
    require(build["source_pcb_sha256"] == PINNED_SOURCE["handbell.kicad_pcb"]
            and build["source_manifest_sha256"] == PINNED_SOURCE["placement-manifest.json"],
            "Build source PCB/manifest identity differs")
    for mapping, base in ((build["source_inputs"], SOURCE), (build["protected_files"], HERE)):
        for name, digest in mapping.items():
            path = check.contained(base, name)
            require(sha(path) == digest, "Stale explicit dependency: " + name)
            require(inputs.get(str(path.relative_to(ROOT))) == digest,
                    "Build dependency absent from explicit checker bindings: " + name)
    validated_inputs, cache = check.validate_cache()
    require(validated_inputs == inputs, "Inputs changed while validating native cache")
    review = read_json(REPORTS / "completion-review.json")
    require(review.get("explicit_input_hashes") == inputs
            and review.get("explicit_native_report_hashes") == cache["reports"]
            and review.get("native_binding_sha256") == sha(REPORTS / "native-input-bindings.json")
            and review.get("hashes", {}).get("handbell.kicad_pcb") == expected
            and review.get("kicad_cli") == cache["tool"],
            "Missing/stale native completion review; finish this candidate's native checker first")
    require(isinstance(review.get("zones"), int), "Incomplete native zone inventory")
    require(set(review["hashes"]) == set(REVIEW_INPUT_NAMES), "Native review input allowlist differs")
    require(all(sha(HERE / name) == review["hashes"][name] for name in REVIEW_INPUT_NAMES),
            "Native review file hashes are stale")
    assert_unchanged(bindings)
    return bindings, review, cache


def assert_unchanged(bindings):
    for name, digest in bindings.items():
        path = ROOT / name
        require(path.resolve().is_relative_to(ROOT), "Input binding escapes the repository")
        require(sha(path) == digest, "Inputs changed during analysis; discard this run: " + name)


def tool_bindings(check, pcb):
    """Hash the selected executable/API and numpy kernel, not ambient directories."""
    core = "_core" if int(np.__version__.split(".")[0]) >= 2 else "core"
    kernel = importlib.import_module(f"numpy.{core}._multiarray_umath")
    extension = importlib.import_module("_pcbnew")
    paths = {
        "python_executable": Path(sys.executable),
        "kicad_cli": check.cli_path(),
        "pcbnew_python": Path(pcb.__file__),
        "pcbnew_extension": Path(extension.__file__),
        "numpy_python": Path(np.__file__),
        "numpy_array_kernel": Path(kernel.__file__),
        "standard_power_symbols": check.cli_path().parent.parent / "share" / "kicad" / "symbols" / "power.kicad_sym",
    }
    return paths, {name: {"file": path.name, "sha256": sha(path)} for name, path in paths.items()}


def assert_tools_unchanged(paths, bindings):
    for name, path in paths.items():
        require(sha(path) == bindings[name]["sha256"], "Tool changed during measurement: " + name)
...
def route_inventory(check, package):
    ...
    require(not any(fp.children("zone") for fp in board.children("footprint")),
            f"{package.name}: footprint zones are unsupported")
    require(not board.children("arc"), "Track arcs require a new native path model")
    ...
    for kind, field in (("segment", "tracks"), ("via", "vias")):
        ...
        for node, row in zip(nodes, rows):
            ...
            require("kind" not in row, "Route data must not override native primitive kind")
            if kind == "segment":
                actual = {}
                for key, value in (("start", row["a"]), ("end", row["b"])):
                    ...
                    actual["a" if key == "start" else "b"] = [v - 100 for v in native]
                ...
                actual.update(layer=node.value("layer"), width=float(node.value("width")))
            else:
                native = list(map(float, node.child("at").atoms()[1:3]))
                require(len(row["xy"]) == 2 and all(math.isfinite(x) for x in row["xy"])
                        and math.dist(native, [x + 100 for x in row["xy"]]) < EPS
                ...
                actual = {"xy": [v - 100 for v in native],
                          "diameter": float(node.value("size")), "drill": float(node.value("drill"))}
            records[uid] = {"kind": kind, "net": node.value("net"), "group": row.get("group"), **actual}
    return records
...
class NativeWalk:
    def __init__(self, check, pcb, package, records):
        ...
        zones = [z for z in self.graph.board.Zones() if not z.GetIsRuleArea()]
        require(all(z.GetNetname() == "GND" and list(z.GetLayerSet().Seq()) == [pcb.F_Cu] for z in zones),
                "Only the checked F-ground fill can be excluded from nonground path measurements")
        self.filled_ground = bool(zones)
        if self.filled_ground:
            from check_printed_bell_power_rework import CopperGraph as PrimitiveGraph
            self.graph = PrimitiveGraph(pcb, package / "handbell.kicad_pcb")
        require(len(self.graph.pads) == 325, "Native inventory must retain all 325 physical pads")
    ...
    def polygon(self, key):
        if key not in self.geometry:
            ...
            for hole_bounds, hole in self.holes:
                if boxes_touch(bounds, hole_bounds):
    ...
    def path(self, name, start, end, role, current=None, layer=None):
        require(layer in (None, "F.Cu", "B.Cu"), "Unsupported path layer")
        ...
        net = next(iter(nets))
        result["net"] = net
        if self.filled_ground and net == "GND":
            native_layer = None if layer is None else (self.pcb.F_Cu if layer == "F.Cu" else self.pcb.B_Cu)
            result.update(
                status="NOT_MODELED_FILLED_GND_NETWORK",
                native_connected=any(self.full_graph.path(a["uuid"],b["uuid"],native_layer) for a in aa for b in bb),
                reason="Actual filled-island connectivity is checked; no 1-D resistance or isolated neck estimate is assigned to the ground plane")
            return result
        ...
        for ai, bi, edge in edges:
            ...
            if edge["kind"] == "overlap":
                contact_width = 2 * min(self.polygon(akey).point_clearance(a),
                                        self.polygon(bkey).point_clearance(b))
                require(math.dist(a, b) < EPS and contact_width > 2 * EPS,
                        "Chosen overlap is not an identical-XY finite-width contact")
                result["walk"].append({
                    "kind": "overlap", "owner_uuid": edge["owner_uuid"],
                    "owner_kind": self.item_kind(edge["owner_uuid"]),
                    "from_owner_uuid": akey[0], "to_owner_uuid": bkey[0],
                    "from_owner_kind": self.item_kind(akey[0]), "to_owner_kind": self.item_kind(bkey[0]),
                    "from_xy_mm": xy(a), "to_xy_mm": xy(b),
                    "from_layer": self.layer_name(akey[1]), "to_layer": self.layer_name(bkey[1]),
                    "length_mm": 0., "contact_inscribed_width_mm": rounded(contact_width),
                    "interpretation": "Finite shared copper portal; zero travel, not an extra resistor",
                })
                continue
            uid = edge["owner_uuid"]
            ...
            if edge["kind"] == "barrel":
                ...
                r = barrel_mohm(drill)
                row.update(drill_mm=rounded(drill), annulus_outer_diameter_mm=rounded(self.barrels[uid][1]),
                           conditional_wall_mm=WALL_MM, conditional_barrel_mOhm=rounded(r))
                barrels += r
                vertical += BOARD_MM
            else:
                ...
                for index in range(count):
                    ...
                    slices.append({"from_xy_mm": xy(c), "to_xy_mm": xy(d),
                                   "length_mm": rounded(math.dist(c, d)),
        ...
        result.update(xy_length_including_pad_travel_mm=rounded(xy_length),
                      ...
                      overlap_portals=sum(s["kind"] == "overlap" for s in result["walk"]),
                      minimum_chosen_strip_width_mm=min(
                          (s["inscribed_width_mm"] for step in result["walk"]
                           for s in step.get("chosen_strip_slices", [])), default=None),
    ...
    def barrel_inventory(self, paths):
        ...
        return [{
            ...
            "xy_mm": xy((self.pcb.ToMM(self.graph.items[uid].GetPosition().x),
                         self.pcb.ToMM(self.graph.items[uid].GetPosition().y))),
            "drill_mm": rounded(drill),
            "annulus_outer_diameter_mm": rounded(diameter),
            "conditional_wall_mm": WALL_MM, "conditional_length_mm": BOARD_MM,
    ...
    def copper_inventory(self, paths):
        selected = defaultdict(set)
        for path in paths:
            for step in path["walk"]:
                for uid in {step["owner_uuid"], step.get("from_owner_uuid"), step.get("to_owner_uuid")} - {None}:
                    selected[uid].add(path["id"])
        rows = []
        for uid, record in sorted(self.records.items()):
            row = {"uuid": uid, "kind": record["kind"], "net": record["net"],
                   "selected_path_ids": sorted(selected[uid]),
                   "within_measured_net_scope": record["net"] in self.built_nets,
                   "author_group_annotation_only": record.get("group"),
                   "classification": "Native net and selected physical walk; no group-derived current"}
            if record["kind"] == "segment":
                row.update(layer=record["layer"], from_xy_mm=record["a"], to_xy_mm=record["b"],
                           drawn_width_mm=record["width"], drawn_length_mm=rounded(math.dist(record["a"], record["b"])))
            else:
                row.update(xy_mm=record["xy"], drill_mm=record["drill"],
                           diameter_mm=record["diameter"], layers=["F.Cu", "B.Cu"])
            rows.append(row)
        return rows
...
def requested_paths(model):
    cases = [
        ...
        ("bulk_to_amp7", "C19.1", "U4.7", "Local bulk land to amplifier pin 7; separate conditional branch", 1),
        ("bulk_to_amp8", "C19.1", "U4.8", "Local bulk land to amplifier pin 8; not added to parallel pin 7", 1),
        ("amp_to_bulk_ground", "U4.THERMAL", "C19.2", "Amplifier to local bulk ground; internal sharing unknown", 1),
        ...
        ("input_cap_return", "C26.2", "U5.4", "Input-capacitor return; pulse/RMS current not assigned", None),
    ...
    for cap in ("C27", "C28"):
        paths += [
            model.path(f"{cap.lower()}_supply", "U5.6", cap + ".1",
                       "Required all-F output-capacitor switching-current corridor", layer="F.Cu"),
            model.path(f"{cap.lower()}_return", cap + ".2", "U5.4",
                       "Required all-F output-capacitor switching return", layer="F.Cu"),
        ]
    return paths
...
def roundtrips(paths, native_shorts):
    ...
    for name, members, currents, target in cases:
        ...
        record = {
            ...
            "connectivity_status": ("INVALID_NATIVE_SHORTS" if native_shorts else
                                    "OPEN" if any(lookup[p]["status"] == "OPEN" for p in members) else
        ...
        if native_shorts:
            record["reason"] = "Native shorts invalidate a round-trip screen; no numerical total is reported"
        elif missing:
            ...
            for current in currents:
                ...
                record["current_cases"].append({
                    ...
                    "numerical_screen_vs_budget": ("WITHIN" if hot <= target else "ABOVE") + "_CHOSEN_WALK_ONLY",
                    "is_target_current": current == currents[0],
                })
            record["reason"] = "Numerical chosen-strip comparison only; actual equivalent drop and component behavior unqualified"
        result.append(record)
    return result


def rough_loop_area(supply, ground):
    if any(p.get("native_electrical_screen_invalid") for p in (supply, ground)):
        return {"area_mm2": None, "reason": "Native shorts invalidate the electrical-loop interpretation"}
    ...
    for cap in ("C27", "C28"):
        ...
        for label, model in (("source", before), ("candidate", after)):
            ...
            for constraint in (None, "F.Cu"):
                variant_name = "unrestricted" if constraint is None else "all_F"
                paths = [
                    model.path(f"{cap}_{label}_{variant_name}_positive", "U5.6", cap + ".1",
                               "Hot-loop positive; pulse/RMS current not assigned", layer=constraint),
                    model.path(f"{cap}_{label}_{variant_name}_return", cap + ".2", "U5.4",
                               "Hot-loop ground; pulse/RMS current not assigned", layer=constraint),
                ]
                if model.graph.shorts:
                    for path in paths:
                        path["native_electrical_screen_invalid"] = True
...
def markdown(report):
    lines = [
        ...
        "OPEN or native-short cases have no invented round-trip total.", "",
        ...
        f"Bound native review: **{report['native_review']['status']}**; "
        f"{len(report['native_review']['errors'])} checker errors; "
        f"{report['native_review']['unconnected_items']} unconnected items.",
        "Native report input/tool hashes and full findings remain in the JSON. "
        "This measurement does not replace or waive completion-review.json/md.", "",
        "With an F-ground fill, GND connectivity uses actual filled islands but its walk, neck and resistance "
        "values are NOT MODELED. Nonground copper is still measured; no track-only return estimate or "
        "invented whole-loop budget is assigned to the plane.", "",
    ...
    for p in report["paths"]:
        ...
        width = p.get("minimum_chosen_strip_width_mm")
        width = f"{width:.2g}" if width is not None else "-"
        lines.append(f"| {p['from']} -> {p['to']} | {p['status']} / {p['layer_constraint']} | {length} | "
                     f"{p.get('barrel_traversals', '-')} | {width} | {value} |")
    ...
    for loop in report["roundtrips"]:
        if not loop["current_cases"]:
            lines.append(f"| {loop['id']} | - | - | {loop['target_mV']} mV @ {loop['target_current_A']} A | "
                         f"{loop['connectivity_status']}: {loop['reason']} |")
    ...
    lines += ["", "Lengths here are inventories, not series resistance. The JSON contains exact native owners, "
              ...
              "is not DRC; USB findings, clearance/short findings, residual islands and native-review gates remain visible.",
              "", "## Method and assumptions", ""]
    if report["native_review"]["errors"]:
        lines += ["### Bound native checker errors", ""]
        lines.extend("- " + error.replace("\n", " ") for error in report["native_review"]["errors"])
        lines.append("")
    ...
    lines += ["", "Inputs were hash-checked before and after the calculation; this is not a cross-process generator lock. "
              ...
              "The measurement JSON SHA-256 embedded below pairs these two outputs.", ""]
    return "\n".join(lines)


def write_reports(report, bindings, tools, identities):
    """Publish only new-package outputs, with explicit sibling staging files."""
    REPORTS.mkdir(parents=True, exist_ok=True)
    json_text = json.dumps(report, indent=2, allow_nan=False) + "\n"
    json_hash = hashlib.sha256(json_text.encode("utf-8")).hexdigest()
    payloads = {
        "power-path-measurements.json": json_text,
        "power-path-measurements.md": markdown(report) + f"\nMeasurement JSON SHA-256: `{json_hash}`\n",
    }
    staged, published = [], []
    digests = {name: hashlib.sha256(content.encode("utf-8")).hexdigest() for name, content in payloads.items()}
    try:
        for name, content in payloads.items():
            path = REPORTS / (name + ".pending")
            with path.open("x", encoding="utf-8", newline="\n") as stream:
                staged.append((path, REPORTS / name))
                stream.write(content)
        assert_unchanged(bindings)
        assert_tools_unchanged(tools, identities)
        for path, target in staged:
            path.replace(target)
            published.append(target)
        assert_unchanged(bindings)
        assert_tools_unchanged(tools, identities)
        require(all(sha(path) == digests[path.name] for path in published), "Published measurement bytes differ")
    except BaseException:
        for path in published:
            if path.exists() and sha(path) == digests[path.name]:
                path.unlink()
        raise
    finally:
...
def main():
    ...
    check = load_checker()
    bindings, review, cache = coherent_inputs(check, args.pcb_sha256.lower())
    before_records, after_records = route_inventory(check, SOURCE), route_inventory(check, HERE)
    require(review["tracks"] == sum(r["kind"] == "segment" for r in after_records.values())
            and review["vias"] == sum(r["kind"] == "via" for r in after_records.values()),
            "Native review/route inventory counts differ")
    assert_unchanged(bindings)
    pcb, api_identity = check.native_api()
    require(api_identity == review["native_api"], "Native API differs from the bound completion review")
    global np
    import numpy as np
    tools, identities = tool_bindings(check, pcb)
    require(identities["kicad_cli"]["sha256"] == cache["tool"]["binary_sha256"]
            and identities["standard_power_symbols"]["sha256"] ==
            cache["tool"]["standard_power_symbol_library_sha256"],
            "Native tool identity changed while preparing measurement")
    before, after = (NativeWalk(check, pcb, SOURCE, before_records),
                     NativeWalk(check, pcb, HERE, after_records))
    native_nets = {"source": before.full_graph.nets(), "candidate": after.full_graph.nets()}
    require(native_nets["source"] == review["connectivity"]["source_native_net_islands"]
            and native_nets["candidate"] == review["connectivity"]["candidate_native_net_islands"]
            and after.full_graph.shorts == review["connectivity"]["native_shape_shorts"],
            "Fresh native copper topology differs from the explicitly bound completion review")
    drc = read_json(REPORTS / "drc.json")
    native_short_findings = [finding for finding in drc["violations"] if "short" in finding["type"].lower()]
    shorts = after.full_graph.shorts + native_short_findings
    assert_unchanged(bindings)
    ...
    if shorts:
        for path in paths:
            path["native_electrical_screen_invalid"] = True
            path.pop("conditional_drop_mV", None)
            path.pop("sensitivity_3A_drop_mV", None)
    assert_unchanged(bindings)
    print("Comparing output-capacitor loops", flush=True)
    comparison = capacitor_comparison(before, after)
    for cap in comparison:
        for variant in cap["candidate"].values():
            if shorts:
                for path in variant["paths"]:
                    path["native_electrical_screen_invalid"] = True
                variant["rough_area"] = rough_loop_area(*variant["paths"])
    assert_unchanged(bindings)
    print("Measuring uncovered neck intervals", flush=True)
    all_candidate_paths = paths + [path for cap in comparison for variant in cap["candidate"].values()
                                   for path in variant["paths"]]
    necks = after.necks(all_candidate_paths)
    def counts(model):
        nets = native_nets["candidate" if model is after else "source"]
    ...
    focus = neck_focus(necks, all_candidate_paths)
    if after.filled_ground:
        for name in ("output_GND_small_pin_exits","amplifier_small_ground_exits"):
            focus.pop(name)
    report = {
        "schema_version": 2, "status": "SCREENING_ONLY_TARGETS_NOT_DEMONSTRATED",
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "candidate_package": str(HERE.relative_to(ROOT)), "source_package": str(SOURCE.relative_to(ROOT)),
        "candidate_pcb_sha256": args.pcb_sha256.lower(), "source_pcb_sha256": sha(SOURCE / "handbell.kicad_pcb"),
        "source_manifest_sha256": PINNED_SOURCE["placement-manifest.json"],
        "source_revision_basis": "Pinned authoritative native inputs; publication pending; no source report-folder snapshot",
        "tool_sha256": sha(__file__), "input_bindings": bindings,
        "tool_bindings": identities, "native_api": api_identity, "native_cli": cache["tool"],
        ...
        "parameters": {"rho_20C_ohm_mm2_per_m": RHO, "nominal_foil_um": FOIL_MM * 1000,
                       ...
                       "cell_planning_A": 2, "cell_sensitivity_A": 3, "five_V_planning_A": 1,
                       "cell_copper_only_target_mV": 50, "five_V_copper_only_target_mV": 100},
        "counts": {"source": counts(before), "candidate": counts(after)},
        "paths": paths, "roundtrips": roundtrips(paths, shorts),
        "capacitor_loop_comparisons": comparison, "exposed_neck_inventory": necks,
        "native_copper_inventory": after.copper_inventory(all_candidate_paths),
        "conditional_barrel_inventory": after.barrel_inventory(all_candidate_paths),
        "neck_focus": focus, "native_shorts": after.full_graph.shorts,
        "ground_plane_measurement_scope": {
            "present": after.filled_ground,
            "filled_islands": getattr(after.full_graph,"zone_islands",[]),
            "model": "Native filled-island connectivity; no equivalent GND resistance or ground neck inventory" if after.filled_ground else "Track/pad/via chosen-walk screen",
            "whole_loop_budgets_closed": False,
        },
        "source_native_shorts": before.graph.shorts,
        "native_drc_short_findings": native_short_findings,
        "native_shorts_block_all_roundtrip_totals": bool(shorts),
        "native_net_islands": native_nets,
        "quiet_terminal_cut_pickoffs": [
            after.full_graph.pickoff("R26", "2", "R27", "2"),
            after.full_graph.pickoff("R24", "2", "C28", "2"),
        ],
        "native_review": {
            "status": review["status"], "errors": review["errors"],
            "unconnected_items": review["native_rules"]["unconnected_items"],
            "routing_complete": review["routing_complete"],
            "native_rules": review["native_rules"],
            "raw_candidate_drc": drc,
            "native_connectivity": review["connectivity"],
            "intentional_net_tie": review["intentional_net_tie"],
            "service_contact_screen": review["service_contact_screen"],
            "geometry": review["geometry"],
            "power_topology": review["power_topology"],
            "release_blockers": review["release_blockers"],
            "report_json_sha256": sha(REPORTS / "completion-review.json"),
            "report_markdown_sha256": sha(REPORTS / "completion-review.md"),
            "interpretation": "Same-input native evidence, not approval and never suppressed by a path budget",
        },
        ...
        "open_gates": [
            ...
            "Bound candidate native review including residual islands, shorts/clearances and USB findings",
            "Raw B contact bases exclude foreign copper; intentional VSS is CELL_NEG, never silently relabeled GND",
            "Exact mechanical rebind; no functional, charging, fabrication, purchase or live-cell approval",
        ],
        "tracking": ["E04/#4", "E05/#5", "E07/#7"],
    }
    assert_unchanged(bindings)
    write_reports(report, bindings, tools, identities)
    print(json.dumps({"status": report["status"], "pcb_sha256": report["candidate_pcb_sha256"],
                      ...
                      "reports": [str(REPORTS / ("power-path-measurements." + ext))
...
if __name__ == "__main__":
    try:
        sys.exit(main())
    except (ValueError, KeyError, TypeError, OSError, RuntimeError, AttributeError, ImportError) as error:
        print(json.dumps({"status": "MEASUREMENT_REFUSED", "error": str(error),
                          "reports_written": False,
                          "action": "Resolve inputs/tools/zones or finish import and native checks; old reports are not this run."}),
              file=sys.stderr)
        sys.exit(1)
